const CourseForumThread = require("../models/CourseForumThread");
...
exports.createForumThread = async (req, reply) => {
  ...
  try {
    const { body, user_id, course_forum_topic_id } = req.body;
    const newForumThread = await CourseForumThread.forge({
      body,
      user_id,
      course_forum_topic_id,
    }).save();
    let newThreadWithUser = await CourseForumThread.where({
...
exports.getAllForumThreads = async (req, reply) => {
  ...
  try {
    const { course_forum_topic_id } = req.params;
    return (
      await CourseForumThread.where({ course_forum_topic_id }).fetchAll({
        withRelated: ["user"],
      })
    ).models;
  } catch (error) {
...
exports.getForumThread = async (req, reply) => {
  ...
  const { id, course_forum_topic_id } = req.params;

  return CourseForumThread.where({ id, course_forum_topic_id })
    .fetch({
      // require: true,
      withRelated: ["user"],
    })
...
exports.updateForumThread = async (req, reply) => {
  ...
  return CourseForumThread.where({ id })
    .fetch({
      require: true,
      withRelated: ["user"],
    })
...
exports.deleteForumThread = async (req, reply) => {
  ...
  try {
    ...
    const thread = await CourseForumThread.where({ id }).fetch({
      require: true,
    });
    ...
    let result = await CourseForumThread.where({ id }).destroy();
