...
class DeleteDiscussionCommentPopup extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    return (
      <>
        {Number(this.props.authUser) === this.props.thread.user.id ? (
          <span
            className=" ml-4 cursors edite"
            onClick={() => {
              this.props.handleDiscussionClick(this.props.thread.id);
              this.openModal();
            }}
          >
        ...
        >
          <DeleteCourseThreadPopup
            ...
            handleDiscussionDelete={this.props.handleDiscussionDelete}
          />
        </Popup>
      </>
    );
...
export { DeleteDiscussionCommentPopup as default };
