...
const files = execSync('find app/api -type f -name "route.ts" | grep "\\[id\\]"', { 
  encoding: 'utf-8',
...
files.forEach((file) => {
  ...
  try {
    ...
    const pattern1 = /export async function (GET|PUT|DELETE|PATCH|POST)\([^,]+,\s*\{\s*params\s*\}:\s*RouteParams\)\s*\{/g;
