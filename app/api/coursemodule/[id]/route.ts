...
export async function PUT(
  ...
  try {
    ...
    if (!hasPermission(user.role, "lms.content.edit")) {
      return createAuthErrorResponse("Insufficient permissions to update course modules", 403);
    }
...
export async function DELETE(
  ...
  try {
    ...
    // Check permissions
    if (!hasPermission(user.role, "lms.content.edit")) {
      return createAuthErrorResponse("Insufficient permissions to delete course modules", 403);
    }
    ...
    if (lessonCount > 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Cannot delete module with existing lessons. Delete lessons first.",
        },
        { status: 400 }
      );
    }
